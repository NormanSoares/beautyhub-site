// Cálculo do preço otimizado para a Oferta Premium - Towel cloth-pink
...
const PREMIUM_DATA = {
    originalPrice: "$0.99", // Preço de compra no AliExpress
...
console.log("🛍️ CÁLCULO DE PREÇO OTIMIZADO - OFERTA PREMIUM");
console.log("=" .repeat(60));

const priceCalc = calculateMinimumProfitablePrice(PREMIUM_DATA.originalPrice);

console.log(`\n${PREMIUM_DATA.productName.toUpperCase()}:`);
console.log(`  💵 Preço original: ${PREMIUM_DATA.originalPrice}`);
console.log(`  💸 Preço original completo: ${PREMIUM_DATA.originalPriceFull}`);
...
console.log(`  📦 Estoque: ${PREMIUM_DATA.stock} unidades`);
...
console.log(`  📦 Lucro total potencial: $${(netProfit * PREMIUM_DATA.stock).toFixed(2)}`);

// Comparação com preço anterior
const oldPrice = 2.48; // Preço anterior calculado
...
const FINAL_PREMIUM_PRICE = {
    originalPrice: PREMIUM_DATA.originalPrice,
    profitPrice: `$${priceCalc.recommended.toFixed(2)}`,
    profitPriceBRL: `R$${(priceCalc.recommended * 5.27).toFixed(2)}`,
    margin: "15%",
    stock: PREMIUM_DATA.stock,
    productName: PREMIUM_DATA.productName,
    netProfit: netProfit,
    profitMargin: profitMargin,
    totalPotentialProfit: netProfit * PREMIUM_DATA.stock
};

console.log("\n🎯 PREÇO FINAL RECOMENDADO PARA IMPLEMENTAÇÃO:");
console.log("=" .repeat(60));
console.log(`💰 Preço otimizado: ${FINAL_PREMIUM_PRICE.profitPrice}`);
console.log(`💎 Preço em BRL: ${FINAL_PREMIUM_PRICE.profitPriceBRL}`);
console.log(`📊 Margem real: ${FINAL_PREMIUM_PRICE.profitMargin.toFixed(2)}%`);
console.log(`💵 Lucro por unidade: $${FINAL_PREMIUM_PRICE.netProfit.toFixed(2)}`);
console.log(`🎯 Lucro total potencial: $${FINAL_PREMIUM_PRICE.totalPotentialProfit.toFixed(2)}`);

// Exportar dados
export const OPTIMIZED_PREMIUM_PRICE = FINAL_PREMIUM_PRICE;

console.log("\n✅ Preço premium otimizado calculado com sucesso!");
